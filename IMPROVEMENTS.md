...
- [x] **Drills marked correct book moves wrong** — 2026-09-17. A FEN reachable by several move orders can have several correct replies in `positions`, but the drill graded against the single canonical `correctSan`, so the other book moves were "wrong". `is_main_line`/`depth` were also unset for all 616 Caro-Kann rows, so that canonical pick fell through to `san ASC` — alphabetical. Fixes: `server/utils/repertoireMoves.ts` + `acceptableSans` on every train-now position (graded in `TrainNowScreen`); `scripts/backfill_main_line.cjs` re-derives the flags from the source PGNs; `scripts/archive/importCaroKann.ts` now writes them; `v2_challenge.ts` no longer deletes the other book replies at a FEN. Proof: `bun run scripts/verify_multi_answer.ts`.
