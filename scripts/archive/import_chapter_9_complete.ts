...
import {
  PgnParser,
  convertParsedGamesToRepertoire,
} from "../src/services/pgn_parser";
...
const PGN_MAPPING: Record<string, string> = {
  "Caro-Kann Chapter 9: Intro & Variation A (6. Bxh6)":
    "BOOK: Ch 9 - Intro & Var A (6. Bxh6)",
...
async function run() {
  ...
  db.run("DELETE FROM chapters WHERE repertoire_id = ? AND name LIKE 'BOOK%'", [
    REPERTOIRE_ID,
  ]);
  ...
  db.transaction(() => {
    for (const parsedGame of parsedGames) {
      ...
      const repData = convertParsedGamesToRepertoire(
        [parsedGame],
        "Temp",
        "black",
      );
