...
const existing = db
  .query("SELECT id FROM repertoires WHERE name LIKE '%Caro%'")
...
if (existing) {
  ...
  const res = db.run(
    ...
    ["Caro-Kann Repertoire", "Black repertoire vs e4", "black"],
  );
...
const transaction = db.transaction(() => {
  ...
  for (const [fen, moves] of Object.entries(importData.positions)) {
    for (const m of moves as any[]) {
      insertPosition.run(repId, fen, m.san, m.nextFen, m.comment || null);
      moveCount++;
    }
  }
