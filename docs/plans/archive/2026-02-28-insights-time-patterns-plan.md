...
if (token.startsWith("{")) {
  if (moves.length > 0) {
    ...
    const clkMatch = commentText.match(
      /\[%clk\s+(\d+):(\d+):(\d+(?:\.\d+)?)\]/,
    );
    if (clkMatch) {
      ...
      moves[moves.length - 1].clockAfter =
...
const pressureThreshold = initialTime ? Math.max(30, initialTime * 0.1) : 30;
...
export function classifyGameShape(
  ...
  const peakLoserAdvantage = Math.max(
    ...evals.map((e) => (loserWasWhite ? e : -e)),
  );
  if (
    peakLoserAdvantage > 200 &&
    Math.sign(finalEval) !== 0 &&
    Math.abs(finalEval) > 100
  ) {
    return "Giveaway";
  }
  ...
  for (let i = 1; i < evals.length; i++) {
    if (
      Math.sign(evals[i]) !== Math.sign(evals[i - 1]) &&
      Math.sign(evals[i]) !== 0 &&
  ...
  const variance =
    evals.reduce((s, e) => s + (e - mean) ** 2, 0) / evals.length;
  ...
  if (evals.length >= 35 && stddev < 120 && totalBlunders <= 1)
    return "Intense";
...
export async function saveAnalysis(req: Request): Promise<Response> {
  try {
    ...
    const gameRow = db
      .query(`SELECT pgn, result, user_color FROM games WHERE id = ?`)
      .get(id) as any;
...
}
{
  move.clockRemaining !== undefined && (
    ...
    >
      ...
      <span className="opacity-60">{formatClock(move.clockRemaining)}</span>
    </span>
  );
}
```
...
export async function runPatternAnalysis(_req: Request): Promise<Response> {
  ...
  if (games.length === 0) {
    return Response.json({
      summary:
        "No analyzed games found. Run analysis on your recent games first.",
      patterns: [],
      action: null,
  ...
  whiteLosses = games.filter(
    (g) => g.user_color === "white" && g.result === "loss",
  ).length;
  blackLosses = games.filter(
    (g) => g.user_color === "black" && g.result === "loss",
  ).length;
  whiteWins = games.filter(
    (g) => g.user_color === "white" && g.result === "win",
  ).length;
  whiteDraws = games.filter(
    (g) => g.user_color === "white" && g.result === "draw",
  ).length;
  blackWins = games.filter(
    (g) => g.user_color === "black" && g.result === "win",
  ).length;
  blackDraws = games.filter(
    (g) => g.user_color === "black" && g.result === "draw",
  ).length;
  ...
  let totalTimeSamplesWhite = 0,
    totalTimeWhite = 0;
  let totalTimeSamplesBlack = 0,
    totalTimeBlack = 0;
  ...
  for (const g of games) {
    ...
    if (!openingMap[opKey])
      openingMap[opKey] = { wins: 0, draws: 0, losses: 0, total: 0 };
    ...
    let moves: any[];
    try {
      moves = JSON.parse(g.analysis_json);
    } catch {
      continue;
    }
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      if (m.timeSpent !== undefined && m.timeSpent !== null) {
        ...
        if (isWhiteMove) {
          totalTimeSamplesWhite++;
          totalTimeWhite += m.timeSpent;
        } else {
          totalTimeSamplesBlack++;
          totalTimeBlack += m.timeSpent;
        }
      }
    }
  }
  ...
  if (!GEMINI_API_KEY) {
    const result = {
      summary:
        "Pattern analysis unavailable — set GEMINI_API_KEY in the server environment.",
      patterns: [],
      action: null,
      stats,
      openings,
  ...
  const topOpenings = openings
    .slice(0, 5)
    .map(
      (o) =>
        `  ${o.name}: ${o.games} games, ${o.winPct}% win, ${o.lossPct}% loss`,
    )
    .join("\n");

  const timeStr =
    avgTimeWhite !== null || avgTimeBlack !== null
      ? [
          avgTimeWhite !== null
            ? `Avg time/move as White: ${avgTimeWhite}s`
            : null,
          avgTimeBlack !== null
            ? `Avg time/move as Black: ${avgTimeBlack}s`
            : null,
...
ACTION: [Single most impactful improvement focus]`;
  ...
  try {
    ...
    const summary =
      extract("SUMMARY", "PATTERN_1") ?? fullText.substring(0, 300);
    const patterns = [
    ...
    const errResult = {
      summary: "Pattern analysis unavailable — check your connection.",
      patterns: [],
      action: null,
      stats,
      openings,
...
function saveReport(report: object) {
  try {
    db.prepare(`INSERT INTO pattern_reports (report_json) VALUES (?)`).run(
      JSON.stringify(report),
    );
    // Keep only the latest 5 rows
    db.query(
      `DELETE FROM pattern_reports WHERE id NOT IN (SELECT id FROM pattern_reports ORDER BY id DESC LIMIT 5)`,
    ).run();
  } catch (e) {
...
```ts
const [patternReport, setPatternReport] = useState<api.PatternAnalysis | null>(
  null,
);
    ...
    <div className="bg-[#0d1117] border border-white/5 rounded-2xl p-5">
      ...
      {patternReport.openings.length === 0 && (
        <p className="text-xs text-slate-600 text-center py-4">
          Analyse more games to see opening stats
        </p>
      )}
    </div>
  );
          ...
          {(
            ...
            return (
              ...
              >
                ...
                <p className="text-[8px] uppercase tracking-wide font-bold opacity-70 truncate px-0.5">
                  {shape}
                </p>
              </div>
            );
      ...
      <div className="bg-[#0d1117] border border-white/5 rounded-2xl p-6 flex flex-col items-center gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs mt-1 max-w-xs">
            AI analysis of recurring weaknesses across all your analyzed games.
            Results are saved until you re-run.
          </p>
        </div>
        <button
          onClick={run}
