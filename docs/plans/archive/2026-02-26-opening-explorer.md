      ...
      <div>
        ...
        {repertoireMoves.length === 0 ? (
          <p className="text-slate-600 text-xs italic leading-relaxed">
            No repertoire moves from this position — you're exploring off-book
            territory.
          </p>
        ) : (
          ...
          <div className="flex flex-col gap-1.5">
            {topLines.slice(0, 3).map((line, i) => {
              ...
              return (
                ...
                >
                  <span className="font-mono text-amber-400 w-10 shrink-0 text-right text-xs">
                    {scoreStr}
                  </span>
...
const modes: { id: TrainingMode; label: string; icon: any; desc: string }[] = [
  {
    ...
    label: "Study",
    icon: GraduationCap,
    desc: "Guided walkthrough: plays moves and shows annotations",
  },
  {
    id: "learn",
    label: "Learn",
    icon: BookOpen,
    desc: "Step-by-step: move is shown, repeat 3× to master",
  },
  {
    id: "full",
    label: "Full Line",
    icon: Play,
    desc: "Master sequences from start to finish",
  },
  {
    id: "weak",
    label: "Weak Spot",
    icon: Target,
    desc: "Drill positions you previously missed",
  },
  {
    id: "quiz",
    label: "Quiz",
    icon: Brain,
    desc: "Random mid-game position challenges",
  },
  {
    id: "explore",
    label: "Explore",
    icon: Compass,
    desc: "Free exploration: browse lines, compare engine suggestions, demo the mainline",
  },
];
```
            ...
            {repertoireMoves.map((move) => {
              ...
              return (
                ...
                >
                  {move.san}
                  {rank && showLines && (
                    <span className="ml-1 text-[8px] text-indigo-400">
                      #{rank}
                    </span>
                  )}
                </button>
          ...
          <div className="mt-2 flex flex-col gap-1">
            {topLines.slice(0, 3).map((line, i) => {
              ...
              return (
                ...
                >
                  <span className="text-[10px] font-mono text-amber-400 w-8 shrink-0 text-right">
                    {scoreStr}
                  </span>
                  <span className="text-[10px] font-mono text-slate-500 truncate">
                    {line.pv.split(" ").slice(0, 5).join(" ")}
                  </span>
                </button>
              );
            })}
