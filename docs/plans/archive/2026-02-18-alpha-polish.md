...
The current `repertoire` mode just sets `showChapters = true`, which opens ChapterLibrary as an overlay _on top of_ the training board. It's not a real mode. Fix: render ChapterLibrary as a full-screen child when `activeMode === 'library'`, same pattern as ReviewTab.
...
const [activeMode, setActiveMode] = useState<
  "training" | "analysis" | "repertoire" | "review" | "lab"
>("training");
// New:
const [activeMode, setActiveMode] = useState<
  "training" | "analysis" | "library" | "review" | "lab"
>("training");
```
...
const handleNavigate = (
  mode: "training" | "analysis" | "repertoire" | "review" | "lab",
) => {
  setActiveMode(mode);
  if (mode === "training") {
...
// New:
const handleNavigate = (
  mode: "training" | "analysis" | "library" | "review" | "lab",
) => {
  setActiveMode(mode);
  if (mode === "training") {
    setShowChapters(false);
  ...
  showChessComModal && (
    <ChessComModal
      ...
      onSync={(username) => {
        ...
        handleAnalyzeLastGame(username);
      }}
      isProcessing={isSyncing}
...
export const PgnImportModal: React.FC<Props> = ({
  onClose,
  onImport,
  isProcessing,
}) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#0d1117] border border-white/10 rounded-[2.5rem] p-8 w-full max-w-lg shadow-2xl">
        ...
        <p className="text-center text-slate-600 text-xs uppercase tracking-widest my-3">
          or paste PGN
        </p>
...
export const ChessComModal: React.FC<Props> = ({
  onClose,
  onSync,
  isProcessing,
  initialUsername,
    ...
    <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#0d1117] border border-white/10 rounded-[2.5rem] p-8 w-full max-w-sm shadow-2xl">
        ...
        <p className="text-slate-400 text-sm mb-6">
          Fetches your most recent rated game.
        </p>
        <input
          type="text"
          value={username}
          ...
          onKeyDown={(e) =>
            e.key === "Enter" && username.trim() && onSync(username.trim())
          }
  ...
  game.deviations.length > 0 && (
    <div className="mt-6 border-t border-white/5 pt-4">
      <p className="text-[10px] font-black uppercase tracking-widest text-rose-400 mb-3 flex items-center gap-2">
        <Target size={12} /> {game.deviations.length} Repertoire Deviation
        {game.deviations.length !== 1 ? "s" : ""}
      </p>
      {game.deviations.map((d, i) => (
        <div
          key={i}
          className="bg-rose-500/5 border border-rose-500/20 rounded-xl p-3 mb-2"
        >
    ...
    async (game: ParsedGame) => {
      ...
      try {
        for (let i = 0; i < moves.length; i++) {
          ...
          const colorToMove: "w" | "b" = move.fenBefore.split(" ")[1] as
            | "w"
            | "b";
...
export const GameLab: React.FC = () => {
  ...
  const evalPoints = useMemo(
    () => reviewedMoves.map((m) => m.cpAfter / 100),
    [reviewedMoves],
  );
      ...
      <div className="flex flex-col h-full bg-[#050507] overflow-hidden font-outfit text-slate-200">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-8 p-8">
          <div className="text-center max-w-sm">
            <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic mb-3">
              Game Review
            </h2>
            <p className="text-slate-400 text-sm leading-relaxed">
              Upload a PGN or sync your latest Chess.com game. Stockfish will
              analyse every move and classify blunders, mistakes, and brilliant
              plays.
            </p>
          </div>
      ...
      <div className="flex-1 flex overflow-hidden flex-col lg:flex-row">
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-[#0d1117]/20 p-4 lg:p-6 gap-4 min-h-0">
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {(
              ["blunder", "mistake", "inaccuracy", "excellent"] as MoveGrade[]
            ).map((grade) => {
              ...
              return (
                <span
                  key={grade}
                  className={cn(
                    "px-3 py-1 rounded-full text-[10px] font-black border border-white/10",
                    color,
                    bg,
                  )}
                >
          ...
          <div className="w-full max-w-[min(60vh,480px)] h-12 bg-black/30 border border-white/5 rounded-xl overflow-hidden relative">
              ...
              {evalPoints.map((cp, i) => {
                ...
                return (
                  <rect
                    ...
                    height={`${heightPct}%`}
                    fill={
                      isWhiteAhead ? "rgba(255,255,255,0.7)" : "rgba(0,0,0,0.7)"
                    }
            ...
            <div className="absolute bottom-1 right-2 text-[9px] text-slate-600 font-mono">
              {BarChart3 && <BarChart3 size={8} className="inline mr-1" />}eval
              graph
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 bg-[#0d1117] border border-white/10 p-1 rounded-xl shadow-xl">
            ...
            <span className="text-xs font-mono text-slate-400 px-2 min-w-[80px] text-center">
              {boardMoveIdx < 0
                ? "Start"
                : `Move ${reviewedMoves[boardMoveIdx]?.moveNumber} (${reviewedMoves[boardMoveIdx]?.color[0].toUpperCase()})`}
            </span>
            <button
              onClick={() =>
                setBoardMoveIdx((i) =>
                  Math.min(reviewedMoves.length - 1, i + 1),
                )
              }
              className="p-2 hover:bg-white/5 rounded-lg text-slate-400 hover:text-white"
            >
            ...
            {reviewedMoves.map((move, i) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={cn(
                      "font-bold text-sm min-w-[48px]",
                      move.color === "white" ? "text-white" : "text-slate-300",
                    )}
                  >
                    {move.san}
                  </span>
...
<button
  ...
  className={cn(
    ...
    stat.label === "Weak Points" && weakCount > 0
      ? "hover:border-rose-500/40 cursor-pointer"
      : "",
  )}
>
  ...
...
```tsx
{
  weakCount > 0 && (
    <div className="mt-8 flex justify-center">
      ...
      >
        <AlertTriangle size={16} /> Drill {weakCount} Weak Position
        {weakCount !== 1 ? "s" : ""}
      </button>
    </div>
  );
...
// NEW:
{
  isError ? (
    <div className="h-full flex flex-col items-center justify-center gap-3">
      <p className="text-rose-400 text-xs font-bold text-center leading-relaxed">
        {currentInsight}
      </p>
      <button
        onClick={onDeepAnalysis}
        className="text-[10px] text-indigo-400 hover:text-indigo-300 uppercase tracking-widest font-black"
      >
        Retry
      </button>
    </div>
  ) : currentInsight ? (
    <div className="...existing insight display...">"{currentInsight}"</div>
  ) : (
