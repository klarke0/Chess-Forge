...
export function listRepertoires(): Response {
  const rows = db
    .query(
      "SELECT id, name, description, side, created_at, updated_at FROM repertoires",
    )
    .all();
...
export function getPositions(repertoireId: number): Response {
  const repertoire = db
    .query("SELECT id FROM repertoires WHERE id = ?")
    .get(repertoireId);
  if (!repertoire)
  ...
  const rows = db
    .query(
      "SELECT fen, san, next_fen, comment, is_main_line, depth FROM positions WHERE repertoire_id = ? ORDER BY is_main_line DESC, depth ASC",
    )
    .all(repertoireId) as Array<{
    fen: string;
    san: string;
    next_fen: string;
    comment: string | null;
  ...
  const positions: Record<
    string,
    Array<{ san: string; nextFen: string; comment?: string; isMainLine: boolean; depth: number }>
  > = {};
...
export async function updateChapterLearnRuns(
  req: Request,
  chapterId: number,
): Promise<Response> {
  ...
  if (typeof body.learnRuns !== "number") {
    return Response.json(
      { error: "learnRuns must be a number" },
      { status: 400 },
    );
  }
  db.prepare(
    "UPDATE chapters SET learn_runs = MAX(learn_runs, ?) WHERE id = ?",
  ).run(body.learnRuns, chapterId);
...
export async function importPgn(req: Request): Promise<Response> {
  ...
  if (!body.positions) {
    return Response.json(
      { error: "positions object is required" },
      { status: 400 },
    );
  }
