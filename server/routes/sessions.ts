/**
 * @legacy V1 session-tracking endpoints. V2 derives session metrics on the
 * fly from the `progress` and `analysis_json` rows; it does not write to
 * the `sessions` table. Preserve until V1 is fully retired.
 */
import db from "../db";
...
export async function createSession(req: Request): Promise<Response> {
  ...
  const result = db
    .prepare("INSERT INTO sessions (repertoire_id) VALUES (?)")
...
export async function endSession(
  ...
  db.prepare(
    ...
    body.positionsDrilled ?? null,
    body.correctCount ?? null,
    body.mistakeCount ?? null,
    sessionId,
  );
