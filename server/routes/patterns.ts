/**
 * @legacy V1 pattern-analysis endpoint. The V1 `InsightsTab` consumes this
 * via `getPatternReport` / `runPatternAnalysis`. V2 Insights uses the
 * `v2_insights.ts` routes instead. Preserve until V1 is fully retired.
 */
import db from "../db";
...
export async function runPatternAnalysis(_req: Request): Promise<Response> {
  ...
  const avgTimeBlack =
    totalTimeSamplesBlack > 0
      ...
      : null;
  const avgCpLoss = (p: PhaseStats) =>
    p.errors > 0 ? Math.round(p.totalCpLoss / p.errors) : 0;
  ...
  if (!GEMINI_API_KEY) {
    const result = {
      summary: "Set GEMINI_API_KEY in server env.",
      patterns: [],
      action: null,
      stats,
      openings,
