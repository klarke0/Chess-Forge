...
export function useGameReview(
  ...
  // never be a blunder/mistake/inaccuracy — eval swings on forced-mate-to-winning
  // transitions cause false positives without this check.
  const isPlayedMoveBest = (fenBefore: string, playedSan: string, bestMoveUci: string): boolean => {
  ...
  const analyzeGame = useCallback(
    async (parsedGame: ParsedGame) => {
      ...
      const pressureThreshold = initialTime
        ? Math.max(30, initialTime * 0.1)
        : 30;
      ...
      try {
        for (let i = 0; i < moves.length; i++) {
          ...
          const fenBefore = game.fen(); // FEN before the move (used for best-move comparison)
          ...
          let normalizedEval =
            game.turn() === "w" ? (score.cp ?? 0) : -(score.cp ?? 0);
          if (score.mate !== null) {
            normalizedEval =
              score.mate > 0
                ? game.turn() === "w"
          ...
          console.log(
            `[Review] Move ${i + 1} (${sideMoved}): ${moveSan}, Eval: ${normalizedEval}, Prev: ${prevEval}, Loss: ${cpLoss}`,
          );
          ...
          if (cpLoss > 150 || score.cp === null) {
            ...
            let deepEval =
              game.turn() === "w" ? (deepScore.cp ?? 0) : -(deepScore.cp ?? 0);
  ...
  const reset = useCallback(() => {
    ...
    setError(null);
    try {
      localStorage.removeItem(getLSKey(gameId));
    } catch {
      /* ignore */
    }
  }, [gameId]);
