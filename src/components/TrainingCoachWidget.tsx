...
const MODE_IDLE_TEXT: Record<string, string> = {
  ...
  learn:
    "Follow along — arrows show the correct move. Tap the wizard to analyze.",
...
export const TrainingCoachWidget: React.FC<TrainingCoachWidgetProps> = ({
  ...
  const { status, message, hint, awaitingNext, mode, learnRunsCompleted } =
    useTrainingStore();
  ...
  const idleText =
    MODE_IDLE_TEXT[mode] ?? "Tap the wizard to analyze this position.";
  const displayText =
    widgetMode === "analysis"
      ? isAnalyzing
        ? message || "The Wizard is contemplating..."
        : currentInsight ||
          "I have no insights for this position yet. Tap the wizard above to ask for a strategic breakdown."
      : message || idleText;
