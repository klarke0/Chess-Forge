...
export const LibraryTab: React.FC<LibraryTabProps> = ({ onSelectChapter }) => {
  ...
  const handleImport = async (
    ...
    try {
      ...
      if (games.length === 0) {
        setImportError(
          "No valid games found in PGN. Check the format and try again.",
        );
        return;
      }
      ...
      setImportError(
        e?.message ?? "Import failed. Check the server and try again.",
      );
    } finally {
      setImporting(false);
    }
  ...
  return (
    <div className="flex flex-col h-full bg-[#050507]">
      ...
      <div className="flex-1 overflow-hidden">
        <ChapterLibrary onSelectChapter={onSelectChapter} onClose={() => {}} />
      </div>
