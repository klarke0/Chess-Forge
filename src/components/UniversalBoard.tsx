...
export const UniversalBoard: React.FC<UniversalBoardProps> = ({
  ...
  const { training, analysis, toggleTrainingVision, toggleAnalysisThreats } =
    useSettingsStore();
  const isTraining =
    mode === "study" ||
    mode === "learn" ||
  ...
  const shouldShowProceed =
    mode !== "study" &&
    (showProceedOverlay !== undefined ? showProceedOverlay : awaitingNext);

  const defaultProceedText =
    status === "complete" && (mode === "weak" || mode === "quiz")
      ? "Next Challenge"
      : "Proceed";
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (mode === "study") {
        ...
        if (e.key === "ArrowLeft" && !e.repeat && moveHistory.length > 0) {
          e.preventDefault();
          onBack?.();
        }
      }
    ...
    <div className="flex flex-col items-center justify-center w-full max-h-full">
      <div className="flex flex-col items-center justify-center relative w-full px-1 py-1 gap-2">
        <div className="flex items-center justify-center relative w-full max-w-full">
          {showEvalBar && (
            <div
              className="w-2.5 mr-2 shrink-0 relative overflow-hidden flex flex-col-reverse rounded-full bg-slate-800 border border-black/50"
              style={{ height: "100%" }}
            >
              <EvalBar />
            </div>
          ...
          <div
            className={cn(
              "relative shadow-[0_20px_50px_-10px_rgba(0,0,0,0.5)] rounded-xl overflow-hidden border-[4px] border-forge-board bg-forge-board shrink-0",
          ...
          >
            ...
            {showVision && (
              <div className="absolute inset-0 z-10 pointer-events-none">
                <VisionOverlay
                  ...
                  playerColor={
                    playerColorProp ?? (repertoireSide as "white" | "black")
                  }
                />
              </div>
            )}
            ...
            {(status === "demo" || status === "simulating") && (
              <div className="absolute top-2 left-2 z-30 bg-indigo-600 text-white px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest shadow-xl flex items-center gap-1.5 animate-pulse border border-indigo-400/30">
                <Eye size={10} />{" "}
                {status === "simulating" ? "Simulating Line" : "Coach Demo"}
              </div>
            )}
        ...
        <div
          className={cn(
            "flex flex-row gap-1.5 shrink-0 justify-center flex-wrap",
        ...
        >
          ...
          <div className="w-full h-[1px] bg-forge-border-subtle my-0.5" />
          <button
            onClick={() =>
              setOrientation((o) => (o === "white" ? "black" : "white"))
            }
            className="p-2.5 rounded-xl bg-forge-card border border-forge-border-default text-slate-400 hover:text-white transition-all shadow-lg"
            title="Flip Board"
          >
            <RotateCw size={14} />
          </button>
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-sm text-slate-300 leading-relaxed">
                        {isCoachAnalyzing
                          ...
                          : currentInsight ||
                            "The Wizard is ready. Request a Deep Analysis to begin."}
                      </p>
                      {!currentInsight &&
                        !isCoachAnalyzing &&
                        onDeepAnalysis && (
        ...
        >
          ...
          <div className="flex flex-col items-center min-w-[100px]">
            ...
            <span className="text-xs font-mono text-indigo-400 font-bold tracking-widest">
              {studyStep} <span className="text-slate-600 mx-1">/</span>{" "}
              {totalStudySteps}
            </span>
          </div>
      ...
      {showLines && topLines.length > 0 && mode !== "explore" && (
        <div
          className={cn(
            "mb-6 px-4 w-full max-w-2xl shrink-0",
            mobileControls && "hidden",
          )}
        >
          <div className="bg-[#0d1117]/60 backdrop-blur-sm border border-white/5 rounded-xl p-3 flex flex-col gap-1.5">
            {topLines.slice(0, 3).map((line, i) => {
              ...
              return (
                ...
                >
                  <span className="font-mono text-indigo-400 w-10 shrink-0 text-right">
                    {scoreStr}
                  </span>
