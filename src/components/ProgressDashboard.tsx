    ...
    <div className="absolute inset-0 z-40 p-12 bg-[#080a0f] overflow-y-auto animate-in fade-in zoom-in-95 duration-500">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-end mb-12 border-b border-white/10 pb-8">
          <div>
            <h2 className="text-5xl font-black text-white uppercase italic tracking-tighter font-outfit">
              Performance Hub
            </h2>
            <p className="text-slate-400 font-bold uppercase tracking-[0.3em] mt-2 text-[10px]">
              Personal Mastery Analytics
            </p>
          </div>
          <button
            onClick={onClose}
            className="px-8 py-3 bg-[#1a1f26] hover:bg-[#252b35] text-slate-200 font-black uppercase tracking-widest text-xs rounded-xl border border-white/10 transition-all"
          >
            Return to Board
          </button>
        </div>
          ...
          {[
            {
              ...
              value: stats
                ? `${Math.round((stats.weeklyAccuracy || 0) * 100)}%`
                : "—",
              icon: TrendingUp,
            ...
            },
            {
              ...
              color: "text-rose-400",
              bg: "bg-rose-500/10",
              onClick: weakCount > 0 ? onDrillWeak : undefined,
            },
          ].map((stat, i) => (
            <div
              key={i}
            ...
            >
              ...
              {stat.onClick && (
                <p className="text-[9px] font-black text-rose-500 uppercase tracking-widest mt-3">
                  Click to drill →
                </p>
              )}
            </div>
        ...
        {stats && (
          <p className="text-slate-600 text-xs text-center mb-8">
            {stats.totalTracked} positions tracked · {totalPositions} in
            repertoire
          </p>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-12 bg-[#11151c] border border-white/10 rounded-[3rem] p-10 shadow-xl">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-300 mb-8 flex items-center gap-3">
              <TrendingUp size={16} className="text-indigo-500" /> Repertoire
              Coverage
            </h3>
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-center mb-3">
                      <span className="font-bold text-white group-hover:text-indigo-400 transition-colors truncate pr-4 text-sm">
                        {chapter.name}
                      </span>
          ...
          <div className="mt-12 bg-rose-600 rounded-[3rem] p-12 flex flex-col md:flex-row items-center justify-between relative overflow-hidden group shadow-2xl shadow-rose-900/20">
            ...
            <div className="relative z-10 text-center md:text-left mb-8 md:mb-0">
              <h3 className="text-3xl font-black text-white uppercase italic leading-none mb-2">
                Targeted Drill Ready
              </h3>
              <p className="text-rose-100 font-bold uppercase tracking-[0.2em] text-xs">
                You have {weakCount} positions requiring immediate review.
              </p>
            </div>
            <button
              onClick={onDrillWeak}
              className="relative z-10 bg-white text-rose-600 px-10 py-5 rounded-[2rem] font-black uppercase tracking-[0.2em] shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
            >
