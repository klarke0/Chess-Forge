...
const MasteryBadge: React.FC<{ mastery: TreeNode["mastery"] }> = ({
  mastery,
}) => {
  const styles = {
    ...
    <div className="select-none">
      <div
        ...
        className={cn(
          ...
          isOpen
            ? "bg-[#1a1f26] border-white/5 shadow-inner"
      ...
      >
        <div className="flex items-center gap-2 min-w-[80px]">
          ...
          <span className="font-mono text-[10px] font-black text-slate-500">
            {Math.floor(depth / 2) + 1}.{depth % 2 === 0 ? "" : ".."}
          </span>
        ...
        <div className="flex-1 flex items-center gap-4 overflow-hidden">
          ...
          {node.comment && (
            <span className="text-[10px] text-slate-400 truncate italic max-w-[400px]">
              "{node.comment}"
            </span>
          )}
        </div>
...
export const RepertoireTree: React.FC<RepertoireTreeProps> = ({
  positions,
  onClose,
  onJumpToPosition,
}) => {
    ...
    <div className="fixed inset-0 z-[90] bg-[#050507] flex font-outfit animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex-1 flex flex-col min-w-0 border-r border-white/5">
        <div className="flex justify-between items-start lg:items-center p-4 lg:p-8 border-b border-white/5 bg-[#0a0d14] shrink-0">
          <div>
            <h2 className="text-2xl lg:text-3xl font-black text-white uppercase italic tracking-tighter">
              Knowledge Tree
            </h2>
            <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-4 mt-2">
              <p className="text-slate-400 font-bold uppercase tracking-[0.3em] text-[10px]">
                Visual Repertoire Explorer
              </p>
              <div className="hidden lg:block h-3 w-[1px] bg-white/10" />
              <div className="flex gap-4">
                <button
                  onClick={() => setGlobalExpand(true)}
                  className="text-[10px] font-bold text-slate-500 hover:text-indigo-400 uppercase tracking-widest flex items-center gap-1"
                >
      ...
      <div className="hidden lg:flex w-[450px] bg-[#080a0f] flex-col items-center justify-center p-8 border-l border-white/5 shadow-2xl relative">
        ...
        <div className="mt-8 text-center">
          ...
          <p className="text-slate-500 text-xs font-medium max-w-[280px] leading-relaxed">
            Hover over any move in the tree to visualize the resulting board
            state instantly.
          </p>
        </div>
      </div>
